...
## 第二层：`push_to_telegram`（信息密度是否够推送，**偏严**）

仅当 `worth=true` 时这一层才生效。决定要不要现在打扰我推 Telegram，还是只默默存档供以后周报趋势分析用。

**核心判断 = max(主帖信息密度, 回帖讨论质量)**

只要**任一**够强就 push，不要简单按回复数判。

### 维度 1：主帖本身的信息密度（与回帖无关）

✅ **主帖独立就够推**（即使 0 回复）：
- 具体面经：面试题目、流程、面试官类型、轮次、问什么（即使主帖是吐槽口吻）
- 具体数字：offer 谈判、TC 拆解、bonus 比例、refresher 节奏
- 内部信息：组的运作、policy 变化、layoff 名单/规模、tooling
- 操作手册级建议：签证流程、跳槽 timing、谈判话术
- 长篇深度复盘：个人决策反思有具体细节支撑
- 行业观察：基于真实数据 / 经历得出的判断

🚫 **主帖单独不够推**：
- 纯情绪宣泄无具体细节（"太难了""破防了""崩溃了"配几句话）
- 模糊感想（"现在工作真不好找"，但没给任何具体数字/案例）
- 重复月经话题且无新角度
- 求资源/求 refer 没附带任何分享
...
✅ **回帖救场**：
- 多条回帖有实质内容（具体数字、内部情况、专业见解）
- 有争议 / 反驳 / 多视角碰撞
- 即使只 1-2 条回帖，但内容信息密度极高（行业老兵爆料、内部人员透露、点破弦外之音）
...
**判断原则**：
- "openai 面试太难了" 如果楼主**写了具体面了什么题/什么流程** → push（面经性质，自带价值）
- "openai 面试太难了" 如果楼主就 5 句话发泄 + 0 回复 → archive
- "Meta E5 谈到 380k 觉得 lowball" 即使无人回，本身就是市价数据点 → push
- "今天 manager 又烦死我了" 没任何细节 → archive
